...
export const getSalesModelData = async (scope) => {
  const {
    manufacturerId,
    distributorId,
    userType
...
export const getSalesWithPartyDetails = async (scope) => {
...
export const getFullInvoiceView = async (invoiceUuid) => {
...
export const getdashboardData = async (scope) => {
  ...
  `;
   ...
   `;
    ...
    dates.map((d) =>
      inv_dates.push(
        new Date(d.invoice_date).toLocaleDateString("en-ca")
      )
    );
    ...
    const [items] = await conn.execute(`
      SELECT i.uuid AS id, i.voucher_number, i.voucher_type, i.party_name, i.place_of_supply, i.state_name, i.invoice_date,le.ledger_group, sk.stock_group,sk.category, itm.stock_item_name,itm.product_code, itm.hsn_code, itm.quantity, itm.rate, itm.discount, itm.amount, itm.billedqty FROM sales i JOIN sales_items itm ON i.id = itm.sales_id join stock_items sk on itm.stock_item_id =sk.id
      LEFT JOIN (SELECT  sle.sales_id,
        GROUP_CONCAT( DISTINCT sle.ledger_name ORDER BY sle.ledger_name ) AS ledgers,
    ...
    const [[openingstock]] = await conn.execute(`SELECT ROUND(SUM(sb.qty), 2) AS openingStock FROM stock_batches sb 
      join stock_items si on si.id= sb.stock_item_id  ${stockWhereClause}
    `, stockParams);
    ...
    const [[purchaseStock]] = await conn.execute(`
      SELECT 
        SUM(itm.quantity) AS purchaseStock
    ...
    const ps = purchaseStock.purchaseStock || 0;
    const ss = stockSold.stocksold || 0;

    const closingStock =
      (Number(os) + Number(ps) - Number(ss)) || 0;
...
export const getFlatSalesReport = async (scope) => {
...
export const getsalesSummary = async (scope) => {
