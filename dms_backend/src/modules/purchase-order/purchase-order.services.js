...
export const createBulkVouchers = async (payload, auth = {}) => {
  ...
  try {
    ...
    const vouchers = (payload.purchaseorders || []).map(normalizeVoucher);
    const invoiceRows = [];
    const partyRows = [];
    const addressRows = [];
    const gstRows = [];
...
const getFlatPurchaseOrders = async (scope) => {
  return purchaseOrderRepository.getFlatPurchaseOrders(scope)
}
const getPoData = async (scope) => {
  return purchaseOrderRepository.getPoData(scope)
}
const getPOSummary = async (scope) => {
  return purchaseOrderRepository.getPOModelData(scope)
};

const getPOPartyDetails = async (scope) => {
  return purchaseOrderRepository.getPOWithPartyDetails(scope)
}

const getFullPOView = async (voucherId) => {
  return purchaseOrderRepository.getFullPOView(voucherId)
}

export default { getFlatPurchaseOrders, getPOSummary, getPOPartyDetails, getFullPOView,getPoData }