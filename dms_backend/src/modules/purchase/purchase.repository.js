...
// export const getvouchersModelData = async (scope) => {
...
//     const [result] = await conn.execute(totalSalesSql, params);
...
//         LEFT JOIN voucher_ledger_entries le
//           ON i.id = le.voucher_id
...
//         const key = `${r.party_type}_${r.party_name_detail}`;

//         if (!inv.parties.has(key)) {
//           inv.parties.set(key, {
//             type: r.party_type,
//             name: r.party_name_detail
//           });
//         }
//       }
...
//         const key = `${r.address_type}_${r.address_line}`;

//         if (!inv.addresses.has(key)) {
//           inv.addresses.set(key, {
//             type: r.address_type,
//             line: r.address_line
//           });
//         }
...
//         const key = `${r.stock_item_name}_${r.quantity}_${r.rate}`;

//         if (!inv.items.has(key)) {
//           inv.items.set(key, {
...
//         const key = `${r.ledger_name}_${r.ledger_amount}_${r.entry_type}`;

//         if (!inv.ledgers.has(key)) {
//           inv.ledgers.set(key, {
//             name: r.ledger_name,
...
//       vouchers: Array.from(map.values()).map(inv => ({
//         ...inv,
//         parties: Array.from(inv.parties.values()),
//         addresses: Array.from(inv.addresses.values()),
//         items: Array.from(inv.items.values()),
//         ledgers: Array.from(inv.ledgers.values())
//       }))
//     };
...
export const getvouchersModelData = async (scope) => {
  ...
  try {
    ...
    for (const p of parties) {
      ...
      if (!voucher.parties.some(x => x.type === p.party_type && x.name === p.name)) {
    ...
    for (const a of addresses) {
      ...
      if (voucher.addresses.some(x => x.type === a.address_type && x.line === a.address_line)) {
...
// iam saying instead of 0 and 1 i want only objects
...
export const getVouchersWithPartyDetails = async (scope) => {
...
export const getFullVoucherView = async (voucherUuid) => {
  const conn = await db.getConnection();

  try {
    /*
     * STEP 1:
     * Fetch voucher using UUID
     */
...
export const getdashboardData = async () => {
  ...
  try {
    ...
    // ITEMS DATA
    const [dates] = await conn.execute(`SELECT invoice_date from invoices`)
    dates.map((d) => inv_dates.push(new Date(d.invoice_date).toLocaleDateString("en-ca")))
    const [items] = await conn.execute(`
        ...
        itm.billedqty
        FROM invoices i
        JOIN invoice_items itm 
        ON i.uuid = itm.invoice_uuid
        `);
    const [[stockSold]] = await conn.execute(`SELECT sum(quantity)as stocksold FROM invoice_items`)
    const [[totl_dist]] = await conn.execute(`SELECT COUNT(DISTINCT l.id)as distributors FROM ledgers l JOIN ledger_entries le ON le.ledger_id = l.id WHERE le.is_party_ledger = true`)
    ...
    const result = rows.map(r => ({
      date: new Date(r.invoice_date).toLocaleDateString("en-ca"),
    ...
    return {
      ...
      total_salesSize: summary.total_sales || 0,
      total_sales_amount: summary.total_sales_amount || 0,
      total_stock_sold: stockSold.stocksold,
      total_distributers: totl_dist.distributors,
...
export const getPoData= async (scope) => {
...
export const getFlatVouchersReport = async (scope) => {
